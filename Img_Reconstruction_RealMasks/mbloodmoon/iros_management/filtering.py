...
def source_filter(n: int | tuple[int]) -> Callable[[np.recarray], np.recarray]:
    ...
    def apply(record: np.recarray) -> np.recarray:
        ...
        Returns:
            output (np.recarray): Output filtered data container.
        """
        sorted_record = np.sort(record, order="NPHOTONS")
        return sorted_record[-n:] if isinstance(n, int) else sorted_record[-n[1] : -n[0]]  #!!!TODO: configure sources idx in sorted rec
